...
const CancelledTableReport = ({ projects, loading, error }) => {
  ...
  const filteredProjects = projects.filter((project) => {
    const cancelledDate = new Date(project.jobCancelledDate);
    const isInDateRange =
      (!startDate || cancelledDate >= startDate) &&
      (!endDate || cancelledDate <= endDate);
    return project.originalStatus === "cancelled" && isInDateRange;
  });
  ...
  const handleCSVDownload = () => {
    const csvData = paginatedProjects.map((project) => ({
      ...
      Reason: project.jobCancellationReason,
      Date: new Date(project.jobCancelledDate).toLocaleString("en-US", {
        hour12: true,
        minute: "2-digit",
        hour: "2-digit",
        day: "2-digit",
    ...
    link.download = "cancelled_projects_report.csv";
    link.click();
  };
  ...
  const handlePDFDownload = () => {
    ...
    const rows = filteredProjects.map((project) => [
      ...
      project.jobCancellationReason,
      new Date(project.jobCancelledDate).toLocaleString("en-US", {
        hour12: true,
        minute: "2-digit",
        hour: "2-digit",
        day: "2-digit",
    ...
    doc.setFontSize(12);
    doc.text("Cancelled Projects Report", 15, 20);
    doc.setFontSize(10);
    ...
    doc.save("cancelled_projects_report.pdf");
  };
        ...
        <table className="table border-b bg-white text-base">
          <thead>
            <tr className="border-b-2 border-secondary-200 text-base text-primary-500">
              ...
              <th className="min-w-[300px]">Services</th>
              <th className="w-full min-w-[200px]">Reason</th>
              <th className="min-w-[200px]">Cancellation Date</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              paginatedProjects.map((project, index) => (
                ...
                >
                  ...
                  <td>{project.jobCancellationReason}</td>
                  <td>
                    {new Date(project.jobCancelledDate).toLocaleString(
                      "en-US",
                      {
                        hour12: true,
                        minute: "2-digit",
...
export default CancelledTableReport;
